...
const colors: {
  light: Record<string, RGBAColor>
  dark: Record<string, RGBAColor>
} = { light: {}, dark: {} }
colorVariants.forEach((variant) => {
  const lightStr = getComputedStyle(html).getPropertyValue(
    `--color-light-${variant}`
  )
...
```vue
<!-- Correct -->
<div class="bg-[var(--theme-100)]"></div>
```
...
const duration =
  parseFloat(getComputedStyle(html).getPropertyValue('--duration-hover')) / 1000
...
<p
  class="ibm-plex-sans-jp-mobile-p1 md:ibm-plex-sans-jp-laptop-p2 text-[var(--theme-text-100)]"
>
  Your content here
</p>
...
<h1
  class="pp-eiko-mobile-h1 md:pp-eiko-desktop-h1 text-[var(--theme-text-100)]"
>
  Large Heading
</h1>
```
