<template>
  ...
  >
    ...
    >
      <!-- Background circle - theme-aware (reactive to theme changes) -->
      <circle
        cx="40"
        cy="40"
        r="40"
        :style="{ fill: 'var(--theme-text-100)', fillOpacity: 0.9 }"
      />

      <!-- "PLAY" text - theme-aware contrasting color (reactive) -->
      <path
 ...
 * Uses CSS variables directly for reactive theme switching.
 *
 * Theme Integration:
 * - Circle background: Uses --theme-text-100 (primary text color) at 90% opacity
 * - "PLAY" text: Uses --theme-100 (main background color) for contrast
 * - CSS variables are reactive - automatically updates when theme changes
 *
 * Features:
 * - Automatic theme adaptation (light/dark) - REACTIVE to theme changes
 ...
 *
 * Technical:
 * - Uses inline :style bindings with CSS variables
 * - Browser evaluates CSS variables → reactive to theme changes
 * - No computed properties needed (CSS handles reactivity)
 *
 * Usage:
 * <CursorPlaySVG />
 */
